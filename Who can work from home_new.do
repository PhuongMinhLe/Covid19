...
// globals to manage users
	...
	global user 2 /* put 1 for Yemen and 2 for Phuong */
...
// globals for trial or real mode
	...
	if $trial ==1 {
		...
		global data_combined $desktop/Data/Combined_Data/sim_combined.dta
		global data_index C:/Users/Dell/Dropbox/COVID-firmes/Working_files/Data
		global data_survey $desktop/Data/enquete_des_individus/echantillon_2000_10_17
		global graphs $desktop/Results/PartA_Descriptive/Graphs
		global tables $desktop/Results/PartA_Descriptive/Tables
	...
	if $trial ==0 {
		...
		global data_combined $INS/Data/Combined_Data/ENPE_combined.dta
		global data_survey $INS/Data/Survey_Data
		global data_task_measures $INS/Data/Task_Measures 
		global graphs $INS/Results/PartA_Descriptive/Graphs
		global tables $INS/Results/PartA_Descriptive/Tables
		global log $INS/Log
		}
...
// *** MPL isco88 if trial mode
merge m:1 isco88 using "$data_index/isco88_rli.dta", keep(1 3) nogen

// *** MPL isco88 if real mode
/*merge m:1 isco08 using "$data_task_measures/isco08_rli.dta" 
...
ren isic3 isic4 // *** MPL trial mode
*merge m:1 nat09 using `nat09_isic4', keep(1 3) nogen // *** MPL real mode
...
// *** MPL Trial mode
gen essential_score = runiformint(0,1)
...
* Set the globals
  ...
  global X30_wagewrk "no_coll male youth couple permanent public urban essind i.qnt_earn"
  global X30_wagewrk_inf "no_coll male youth couple formal urban essind i.qnt_earn"
  global X40_wagewrk "no_coll male age40 couple permanent public urban essind i.qnt_earn"
  global X40_wagewrk_inf "no_coll male age40 couple formal urban essind i.qnt_earn"
  global X50_wagewrk "no_coll male age50 couple permanent public urban essind i.qnt_earn"
  global X50_wagewrk_inf "no_coll male age50 couple formal urban essind i.qnt_earn" 
...
* Descriptive analysis on low_rli and high_rli
  ...
  table edu sex	  [aw=wgt], c(mean rli mean teleworkable mean earn) format(%4.2f) row
  table contract sex  [aw=wgt], c(mean rli mean teleworkable mean earn) format(%4.2f) row
  table skill sex 	  [aw=wgt], c(mean rli mean teleworkable mean earn) format(%4.2f) row
  table public sex    [aw=wgt], c(mean rli mean teleworkable mean earn) format(%4.2f) row
  table youth sex     [aw=wgt], c(mean rli mean teleworkable mean earn) format(%4.2f) row
  table urban sex     [aw=wgt], c(mean rli mean teleworkable mean earn) format(%4.2f) row
  table position  sex [aw=wgt], c(mean rli mean teleworkable) format(%4.2f) row
...
foreach X in X30 X40 X50 {
  ...
  reg low_rli $`X' i.gouv i.isic4_reduced [pw = wgt] if selfemp == 1, vce(cluster isco88)
  reg low_teleworkable $`X' i.gouv i.isic4_reduced [pw = wgt] if selfemp == 1, robust
  reg low_teleworkable $`X' i.gouv i.isic4_reduced [pw = wgt]  if selfemp == 1, vce(cluster isco88)
  reg low_rli $`X' i.gouv i.isic4_reduced [pw = wgt] if hhwrk == 1, robust
  reg low_rli $`X' i.gouv i.isic4_reduced [pw = wgt] if hhwrk == 1, vce(cluster isco88)
  reg low_teleworkable $`X' i.gouv i.isic4_reduced [pw = wgt] if hhwrk == 1, robust
  reg low_teleworkable $`X' i.gouv i.isic4_reduced [pw = wgt] if hhwrk == 1, vce(cluster isco88)
  }

 foreach X in X30_wagewrk X30_wagewrk_inf X40_wagewrk X40_wagewrk_inf X50_wagewrk X50_wagewrk_inf{
  reg low_rli $`X' i.gouv i.isic4_reduced [pw = wgt] if wagewrk == 1, robust
  reg low_rli $`X' i.gouv i.isic4_reduced [pw = wgt] if wagewrk == 1, vce(cluster isco88)
  reg low_teleworkable $`X' i.gouv i.isic4_reduced [pw = wgt] if wagewrk == 1, robust
  reg low_teleworkable $`X' i.gouv i.isic4_reduced [pw = wgt] if wagewrk == 1, vce(cluster isco88)
  }
